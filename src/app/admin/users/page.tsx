...
export default function UsersListPage() {
    ...
    const toggleBdmStatus = async (id: string, currentStatus: boolean) => {
        setUpdatingId(id);
        try {
            const { error } = await supabase
                .from('profiles')
                // @ts-expect-error type generated by CLI hasn't fully propagated or handles booleans strictly
                .update({ is_bdm: !currentStatus })
                .eq('id', id);

            if (error) throw error;
            setProfiles(prev => prev.map(p => p.id === id ? { ...p, is_bdm: !currentStatus } : p));
        } catch (err) {
            console.error("Error updating BDM status:", err);
            alert("Failed to update BDM status. Please try again.");
        } finally {
            setUpdatingId(null);
        }
    };
                                    ...
                                    <div className="mt-auto pt-6 border-t border-slate-50 flex flex-col gap-4">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex flex-col">
                                                <span className="text-sm font-black text-slate-900">BDM Status</span>
                                                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Display as Expert</span>
                                            </div>
                                            <button
                                                onClick={() => toggleBdmStatus(profile.id, !!profile.is_bdm)}
                                                disabled={updatingId === profile.id}
                                                className={`relative inline-flex h-7 w-12 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${profile.is_bdm ? 'bg-blue-600' : 'bg-slate-200'} disabled:opacity-50`}
                                            >
                                                <span className="sr-only">Toggle BDM status</span>
                                                <span
                                                    className={`pointer-events-none inline-block h-6 w-6 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${profile.is_bdm ? 'translate-x-5' : 'translate-x-0'}`}
                                                />
                                            </button>
                                        </div>
